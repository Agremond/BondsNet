    ...
    public partial class FormMain : Form
    {
       ...
       // FuturesClientHolding futuresPosition;
        ...
        void Run()
        {
            ...
            if (tools[secCodeindex].currACY != -999)
            {
                textBoxACY.Text = Convert.ToString(tools[secCodeindex].currACY + "%");
            }
        ...
        void GetIndicators()
        {
            ...
            for ( int i = 0; i <= tools.Capacity;i++)
            {
                ...
                if (toolsOrderBook != null)
                {
                    ...
                    {
                        bid = Convert.ToDecimal(toolsOrderBook[i].bid[toolsOrderBook[i].bid.Count() - 1].price);
                    ...
                    if (toolsOrderBook[i].offer != null)
                    {
                        ...
                        offer = Convert.ToDecimal(toolsOrderBook[i].offer[0].price);
                ...
                {
                    tools[i].currACY = Convert.ToDouble( Math.Round(((364 / couponPeriod) * coupon / value) / (offer / 100), 5) * 100);
        ...
        {
            // при подходящей дходности отправляем заявку на покупку
            foreach (Tool tool in tools)
            {
             //   if (tool.currACY >= tool
            }
