...
namespace QuikSharpDemo
{
   ...
   //         Graphics graf = picGraph.CreateGraphics();
                ...
                if(positions[i].toolQty == 0 && positions[i].State == State.Waiting)
                {
                    if (tools[i].CurrentACY >= tools[i].GoalACY) // при подходящей доходности отправляем заявку на покупку
                    {
                        if(tools[i].Offer > 0)
                        {
                            ...
                            EntrancePosition(Operation.Buy, priceEntrance, settings.QtyOrder, tools[i]);
        ...
        void EntrancePosition(Operation operation, decimal price, int qty, Tool tool)
        {
          ...
          //  {
                ...
                else
                {
                    ...
                    NewPos2Table(tool.Name, positions[i].toolQty, positions[i].dateTimeEntrance, positions[i].priceEntrance, tool);
                }
        ...
        void NewPos2Table(string toolName, int qty, DateTime timeEntr, decimal price, Tool tool)
        {
            ...
            switch (selectedCommand)
            {
                ...
                case "Выставить заявку (без сделки)":
                    try
                    {
                        ...
                        EntrancePosition(Operation.Buy, priceInOrder, settings.QtyOrder, tools[secCodeindex]);
